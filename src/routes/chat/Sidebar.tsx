          ...
          return (
            <li key={index}>
                <a href={RoutePaths.CHAT + "/" + chat.id} className="message-row">
                    <div className={`notification ${!chat.what.seen ? "notification-active" : ""}`}>
                        <span className="sr-only" aria-live="assertive">{chat.what.seen ? "Read": "Unread"}</span>
                    </div>
                    <div className="avatar">
                    ...
                    <div className="contents">
                        ...
                        <div className="meta">
                            <p className={!chat.what.seen ? "text-highlight" : ""}>{chat.what.timestamp}</p>
                            {chat.what.from === "self" && chat.what.seen && <p className="text-xxs">Seen</p>}
                        </div>
                    </div>
                </a>
            </li>
...
function convertTime(time: string) {
  ...
  if (timeDifference < oneDay && inputTime.getDate() === now.getDate()) {
      const formattedTime = `${inputTime.getHours()}:${String(inputTime.getMinutes()).padStart(2, '0')}`;
      ...
      const formattedDate = `${inputTime.getFullYear()}-${String(inputTime.getMonth() + 1).padStart(2, '0')}-${String(inputTime.getDate()).padStart(2, '0')}`;
